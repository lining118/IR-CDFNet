...
from utils.loss import PixLoss, FLoss
from dataset import MyData
from models.baseline import FDNet
...
def check_data(batch):
    inputs, inputs_ycbcr, gts = batch
    if torch.isnan(inputs).any() or torch.isinf(inputs).any():
        raise ValueError("输入数据中包含NaN或无穷大值")
    if torch.isnan(inputs_ycbcr).any() or torch.isinf(inputs_ycbcr).any():
        raise ValueError("输入数据YCBCR中包含NaN或无穷大值")
    if torch.isnan(gts).any() or torch.isinf(gts).any():
        raise ValueError("目标数据中包含NaN或无穷大值")
...
def init_models_optimizers(epochs, to_be_distributed):
    model = FDNet(backbone_pretrained=True)
    if args.resume:
...
class Trainer:
    ...
    def _train_batch(self, batch, epoch):
        ...
        inputs_ycbcr = batch[1].to(device)
        gts = batch[2].to(device)

        # Use autocast to enable mixed precision training
        with autocast():
            output_1, output_2, output_3, output_4 = self.model(inputs, inputs_ycbcr)
            # 检查模型输出
            # check_tensor(pred_m, "pred_m")

            # Losses with adjusted weights
            if epoch>20: weight_4 = 2.0
            else : weight_4 = 1.0

            loss_pix1 = self.pix_loss(output_1, torch.clamp(gts, 0, 1)) * 1.0
            loss_pix2 = self.pix_loss(output_2, torch.clamp(gts, 0, 1)) * 1.0
            loss_pix3 = self.pix_loss(output_3, torch.clamp(gts, 0, 1)) * 1.0
            loss_pix4 = self.pix_loss(output_4, torch.clamp(gts, 0, 1)) * weight_4

            # Average loss with adjusted weight
            total_weight = 3.0 + weight_4  # The sum of the weights
            loss_pix = (loss_pix1 + loss_pix2 + loss_pix3 + loss_pix4) / total_weight
            self.loss_dict['loss_pix'] = loss_pix.item()
            loss = loss_pix
            self.loss_dict['loss_pix'] = loss.item()
