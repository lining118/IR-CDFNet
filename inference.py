...
def inference(model, data_loader_test, pred_root, method, testset, device=0):
    ...
    for batch in tqdm(data_loader_test, total=len(data_loader_test)) if 1 or config.verbose_eval else data_loader_test:
        ...
        with torch.no_grad():
            _, _,_,output = model(inputs, inputs_ycbcr)
            output = torch.sigmoid(output)

        os.makedirs(os.path.join(pred_root, method, testset), exist_ok=True)

        for idx_sample in range(output.shape[0]):
            # print('pred[idx_sample].shape={}'.format(pred[idx_sample].shape))
            res = nn.functional.interpolate(
                output[idx_sample].unsqueeze(0),
            ...
            # print('binary_res[idx_sample].shape={}'.format(binary_res[idx_sample].shape))
            ...
            #save_tensor_img(res_e, os.path.join(os.path.join(pred_root, method, testset,'pred_e'), label_paths[idx_sample].replace('\\', '/').split('/')[-1]))   # test set dir + file name

        '''    
        # 结构权重可视化
        for idx_sample in range(len(fd)):
            # print('pred[idx_sample].shape={}'.format(pred[idx_sample].shape))
            # fd[idx_sample] = fd[idx_sample].sigmoid()
            res = nn.functional.interpolate(
                fd[idx_sample],
                size=cv2.imread(label_paths[0], cv2.IMREAD_GRAYSCALE).shape[:2],
                mode='bilinear',
                align_corners=True
            )
            res = torch.sigmoid(res)
            # print('fd[idx_sample] = {}'.format(res))
            threshold_0 = 0.15
            threshold_1 = 0.5
            threshold_2 = 0.5

            if idx_sample == 0:
                binary_res = torch.where(res < threshold_0, torch.tensor(0.0), res)
            if idx_sample == 1:
                binary_res = torch.where(res < threshold_1, torch.tensor(0.0), res)
            if idx_sample == 2:
                binary_res = torch.where(res < threshold_2, torch.tensor(0.0), res)

            # print('res.shape = {}'.format(res.shape))
            # print('binary_res[idx_sample].shape={}'.format(binary_res[idx_sample].shape))
            save_tensor_heatmap(res, os.path.join(os.path.join('./testfolder', 'd{}'.format(idx_sample)), label_paths[0].replace('\\', '/').split('/')[-1]))
            '''
    if model_training:
        model.train()
    return None
...
def main(args):
    ...
    if config.model == 'SANet':
        model = FDNet(backbone_pretrained=False)
    weights_lst = sorted(
