...
class MyData(data.Dataset):
    def __init__(self, datasets, image_size, is_train=True):
        ...
        if self.load_all:
            self.images_loaded, self.images_ycrcb_loaded, self.labels_loaded = [], [], []
            ...
            for image_path, label_path in tqdm(zip(self.image_paths, self.label_paths), total=len(self.image_paths)):
                ...
                _image_ycrcb = path_to_image(image_path, size=(config.train_size, config.train_size), color_type='rgb2ycrcb')
                _label = path_to_image(label_path, size=(config.train_size, config.train_size), color_type='gray')
                self.images_loaded.append(_image)
                self.images_ycrcb_loaded.append(_image_ycrcb)
    ...
    def __getitem__(self, index):
        ...
        if self.load_all:
            ...
            image_ycrcb = self.images_ycrcb_loaded[index]
            ...
            image = path_to_image(self.image_paths[index], size=(config.train_size, config.train_size),
                                  color_type='rgb')
            image_ycrcb = path_to_image(self.image_paths[index], size=(config.train_size, config.train_size),
                                  color_type='rgb2ycrcb')
        ...
        #if self.is_train:
           # image, image_ycrcb, label,  = preproc(image, image_ycrcb, label, preproc_methods=config.preproc_methods)
        ...
        image, image_ycrcb, label = self.transform_image(image), self.transform_image(image_ycrcb),self.transform_label(label)


        if self.is_train:
            return image, image_ycrcb, label  # , class_label
        else:
            return image, image_ycrcb, label, self.label_paths[index]
