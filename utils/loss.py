...
class PixLoss(nn.Module):
    ...
    def __init__(self):
        ...
        if 'bce' in self.lambdas_pix_last and self.lambdas_pix_last['bce']:
            ...
            # Use binary_cross_entropy_with_logits instead of BCELoss for amp autocast
    ...
    def forward(self, pred, gt):
        ...
        if pred.shape != gt.shape:
            pred = nn.functional.interpolate(pred, size=gt.shape[2:], mode='bilinear',
                                             align_corners=True)
        pred = pred.sigmoid()
        for criterion_name, criterion in self.criterions_last.items():
            _loss = criterion(pred, gt) * self.lambdas_pix_last[criterion_name]
            loss += _loss
...
class FLoss(torch.nn.Module):
    def __init__(self):
        ...
        self.quantization_table_Y = torch.tensor([
        ...
        # 色度分量的量化表 (8x8)
        self.quantization_table_C = torch.tensor([
    ...
    def forward(self, Y, M, x, q_table='Y'):
        if q_table == 'Y':
            q = self.quantization_table_Y
        elif q_table == 'C':
            q = self.quantization_table_C
        else:
            raise ValueError("q_table must be 'Y' or 'C'")

            # Ensure q has the correct dimensions
        q = q.to(Y.device)

        # 计算 DCT
        dct_Y = DCT.dct_2d(x * Y, norm='ortho')
        dct_M = DCT.dct_2d(x * M, norm='ortho')

        # 计算 DCT 的差值
        dct_diff = dct_Y - dct_M

        # 计算差值的二范数的平方
        norm_squared = torch.norm(dct_diff) ** 2

        # 除以量化表 q
        result = norm_squared / q.sum()  # 使用量化表 q 的总和进行归一化
